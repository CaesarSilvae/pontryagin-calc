...
function [totDerivArr,totDerivCoeffArr] = ...
    ...
    % % if evenDoublewArr and evenCoeffArr do not have
    % % the same length, give error
    % if evenNum ~= length(evenCoeffArr)
    %     toLog(-1,'Length of evenCoeffArr is not the same as even SingleArr');
    %     toLog(1,['D-' num2str(dim) ... 
    ...
    % the matrix equation
    K = solveEqn(evenCoeffMat,oddCoeffMat,dim, ...
        partialNum);
    ...
    if isempty(K)
        ...
        if ~rowFound
            % write to log file 
            toLog(-1,'K''*Meven did not yield Pontryagin!');
            toLog(1,{'Modd',oddCoeffMat,'K',K, ...
                'K''*Meven',Keven});
            toLog(1,['D-' num2str(dim) ... 
                ', # of (dw)''s: ' num2str(partialNum)]);
...
% the matrix equation
function K = solveEqn(Meven,Modd,dim,partialNum)
    ...
    if size(Modd,1) ~= eqnNum
        toLog(-1,'# of rows of Meven and Modd are not the same!');
        toLog(1,{'Meven',Meven,'Modd',Modd});
        toLog(1,['D-' num2str(dim) ... 
            ', # of (dw)''s: ' num2str(partialNum)]);
    ...
    % % coefficient matrix (eq. 4.127 in thesis)
    % Mred = rref(Modd');
    ...
    if ~isequal(KModd,zeros(size(KModd)))
        % write to log
        toLog(-1,'Odd terms do not vanish!');
        toLog(1,{'Modd',Modd,'K',K});
        toLog(1,['D-' num2str(dim) ... 
            ', # of (dw)''s: ' num2str(partialNum)]);

        % save K matrix
        saveMat(dim,partialNum,{'K',K},true);
    ...
    % save K matrix
    saveMat(dim,partialNum,{'K',K},false);
end
